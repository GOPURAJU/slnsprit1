...
def personal_detail_view(request, instance_id=None):
    ...
    return render(request, 'admin/personal_detail_form.html', {'form': form, 'random_number': random_number})
...
<<<<<<< HEAD
def customer_profile_add(request, instance_id=None):
    customerprofile = get_object_or_404(CustomerProfile, id=instance_id) if instance_id else None
    if request.method == 'POST':
        form = CustomerProfileForm(request.POST, request.FILES, instance=customerprofile)
        if form.is_valid():
            customerprofile = form.save()
            print(f"Redirecting to 'applicant_document_create' with id: {customerprofile.id}")  # Debug print
            return redirect('applicant_document_create', instance_id=customerprofile.id)
        else:
            print(f"Form errors: {form.errors}")  
    else:
        form = CustomerProfileForm(instance=customerprofile)

    return render(request, 'main/home_add.html', {'form': form, 'random_number': customerprofile.random_number if customerprofile and customerprofile.random_number else None})
=======
def customer_profile_view(request, instance_id=None):
    ...
    return render(request, 'admin/customer_profile_form.html', {'form': form, 'random_number': customer_profile.random_number if customer_profile and customer_profile.random_number else None})
>>>>>>> origin/master
...
def update_customer_profile_view(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('', instance_id=customer_profile.id )
    else:
        form = CustomerProfileForm(instance=customer_profile)
